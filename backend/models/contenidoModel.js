...
const agregarContenido = async (contenido) => {
    const query = 'INSERT INTO Contenidos (tipo, titulo, url_contenido, duracion) VALUES (?, ?, ?, ?)';
    const { tipo, titulo, url_contenido, duracion } = contenido;
    ...
    try {
        const [result] = await db.query(query, [tipo, titulo, url_contenido, duracionValida]);
...
const actualizarContenido = async (id, contenido) => {
    const query = 'UPDATE Contenidos SET tipo = ?, titulo = ?, url_contenido = ?, duracion = ? WHERE id_contenido = ?';
    const { tipo, titulo, url_contenido, duracion } = contenido;

    try {
        const [result] = await db.query(query, [tipo, titulo, url_contenido, duracion, id]);
